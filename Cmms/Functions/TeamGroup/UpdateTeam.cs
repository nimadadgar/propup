...
namespace Cmms
{
    ...
    public class UpdateTeam
    {
        ...
        public UpdateTeam(ILoggerFactory loggerFactory,
       ...
       [Function("UpdateTeamGroup")]
        ...
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post",Route = "teamgroup/update")]
        HttpRequestData req,
            FunctionContext executionContext)
        {

           var requestObject=await req.GetBodyAsync<UpdateTeamGroupDto>();
            if (!requestObject.IsValid)
            {
                return req.BadResponse("Required some field please fill in", HttpStatusCode.BadRequest, requestObject.ValidationResults);
            }

            var result = await  _teamService.UpdateTeam(requestObject.Value);
